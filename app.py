...
def insertar_prediccion(l_p, l_s, a_s, a_p, prediccion):
    try:
        ...
        cur  = conn.cursor()
        cur.execute(
            "INSERT INTO tb_iris (l_p, l_s, a_s, a_p, prediccion) VALUES (%s, %s, %s, %s, %s)",
...
def obtener_historico():
    try:
        ...
        cur.execute("SELECT id, created_at, l_p, l_s, a_s, a_p, prediccion FROM tb_iris ORDER BY created_at DESC")
        rows = cur.fetchall()
        cur.close()
        conn.close()
        return rows
